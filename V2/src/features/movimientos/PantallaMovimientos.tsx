...
export function PantallaMovimientos() {
  ...
  const { visual, idPorToken } = usarCategorias();
      ...
      {hayFiltrosActivos && (
        <div className="movimientos-chips-activos">
          ...
          {Array.from(categoriasSeleccionadas).map((idMock) => {
            const cat = listaCategorias.find((c) => idPorToken(c.id) === idMock);
            return (
            ...
            <div className="movimientos-hoja__chips">
              {listaCategorias.map((cat) => (
                <ChipCategoria
                  key={cat.id}
                  categoria={cat}
                  seleccionada={categoriasSeleccionadas.has(idPorToken(cat.id) ?? "")}
                  onClick={() => {
                    const idReal = idPorToken(cat.id);
                    if (idReal) alternarCategoria(idReal);
                  }}
                />
              ))}
            </div>
